...
namespace EventStore.Core.Authentication
{
    public class InternalAuthenticationProvider
        : IAuthenticationProvider, IHandle<InternalAuthenticationProviderMessages.ResetPasswordCache>
    {
        ...
        public void Authenticate(AuthenticationRequest authenticationRequest)
        {
            ...
            {
                if (authenticationRequest.TrustedWithoutPassword || cached.Item1 != null)
                {
                    AuthenticateWithPasswordOrTrusted(authenticationRequest, cached.Item1, cached.Item2);
                    return;
                }
            }
            ...
            AuthenticationRequest authenticationRequest)
        {
            try
            {
                ...
                if (userData.Disabled)
                    ...
                    AuthenticateWithPasswordHashOrTrusted(authenticationRequest, userData);
            }
            catch
            {
                authenticationRequest.Unauthorized();
        ...
        private void AuthenticateWithPasswordHashOrTrusted(
            AuthenticationRequest authenticationRequest,
            UserData userData)
        {
            if (
                !(authenticationRequest.TrustedWithoutPassword
                  || _passwordHashAlgorithm.Verify(authenticationRequest.SuppliedPassword, userData.Hash, userData.Salt)))
            {
                authenticationRequest.Unauthorized();
                return;
            }
        ...
        private void AuthenticateWithPasswordOrTrusted(
            AuthenticationRequest authenticationRequest,
            ...
            IPrincipal principal)
        {
            if (
                !(authenticationRequest.TrustedWithoutPassword
                  || authenticationRequest.SuppliedPassword == correctPassword))
            {
                authenticationRequest.Unauthorized();
                return;
            }
